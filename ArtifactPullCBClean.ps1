...
#
## The script is forked and modified from the original version of script by Jeff Rotenberger 
...
## ----------------------------------------------------------------------------------------------------------------------------------------		
##  COLLECT EACH USERS Activities Cache Database
## ----------------------------------------------------------------------------------------------------------------------------------------
		#Set User path variable
		
Write-Host "User Activities Cache Database"
		
		
		
				
date | select DateTime | ConvertTo-html -Body "<H2> Pulling ActivitiesCache.db files.... </H2>" >> $OutLevel1

$localprofiles = Get-WMIObject Win32_UserProfile -filter "Special != 'true'" -ComputerName $target | Where {$_.LocalPath -and ($_.ConvertToDateTime($_.LastUseTime)) -gt (get-date).AddDays(-15) }
foreach ($localprofile in $localprofiles){
	$temppath = $localprofile.localpath
	$source = $temppath + "\appData\local\ConnectedDevicesPlatform"
	$eof = $temppath.Length
	$last = $temppath.LastIndexOf('\')
	$count = $eof - $last
	$user = $temppath.Substring($last,$count)
	$destination = "$dest\users\" + $user + "\Activities\"

Write-Host "  Copying $source to $destination"		
If ($OSArch -eq '32')
	{
	$command = 'robocopy.exe $source $destination /copyall /s'
	iex "& $command"
	}
ElseIf ($OSArch -eq '64')
	{
...
If ($OSArch -eq "64")
	{
	$command = '$tools\RawCopy64.exe /FileNamePath:c:0 /OutputPath:$dest'
	iex "& $command"
...
If ($OSArch -eq "64")
	{
	$command = '$tools\RawCopy64.exe /FileNamePath:c:2 /OutputPath:$dest'
	iex "& $command"
	}
Else
	{
	$command = '$tools\RawCopy.exe /FileNamePath:c:2 /OutputPath:$dest'
	iex "& $command"
	}
...
$command = '$tools\autorunsc.exe -a *  >> $dest\autoruns\$target-autostarting-locations.txt'
iex "& $command"
...
$scheduledtasks1=@("C:\Windows\Tasks\", "C:\Windows.old\Windows\Tasks\")
$scheduledtasks2=@("C:\Windows\", "C:\Windows.old\Windows\")
$scheduledtasks3=@("C:\Windows\System32\Tasks\","C:\Windows\syswow64\Tasks\", "C:\Windows.old\Windows\System32\Tasks\")

foreach ($scheduledtask in $scheduledtasks1){
	$source = $scheduledtask
	$destination = "$dest\autoruns\"
	$command = 'robocopy.exe $source $destination /ZB /copy:DAT *.job /r:0 /ts /FP /np /log:$dest\autoruns\tasks-robocopy-log1.txt'
	iex "& $command"
	}
foreach ($scheduledtask in $scheduledtasks2){
	$source = $scheduledtask
	$command = 'robocopy.exe $source $destination /ZB /copy:DAT SchedLgU.txt /r:0 /ts /FP /np /log:$dest\autoruns\tasks-robocopy-log2.txt'
	iex "& $command"
	}
foreach ($scheduledtask in $scheduledtasks3){
	$source = $scheduledtask
	$command = 'robocopy.exe $source $destination /E /ZB /copy:DAT /r:0 /ts /FP /np /log:$dest\autoruns\tasks-robocopy-log3.txt'
	iex "& $command"
	}


		
$command = 'robocopy.exe $env:WINDIR\Tasks $dest\autoruns\ /ZB /copy:DAT /r:0 /ts /FP /np /log:$dest\autoruns\tasks-robocopy-log.txt'
iex "& $command"
...
$gpp1=@("C:\Windows\System32\grouppolicy\", "C:\ProgramData\Microsoft\Group Policy\History\", "C:\Users\%user%\AppData\Local\Microsoft\Group Policy\History","C:\Windows\System32\grouppolicy\*\Scripts\", "C:\Windows.old\Windows\System32\grouppolicy\*\Scripts\")
$gpp2=@("C:\Windows\System32\grouppolicy\", "C:\Windows.old\Windows\System32\grouppolicy\")
$gpp3=@("C:\Windows.old\Windows\System32\grouppolicy\")

$destination = "$dest\group_policy\"
foreach ($gpp in $gpp1){
	$source = $gpp
	$command = 'robocopy.exe $source $destination /E /ZB /copy:DAT /r:0 /ts /FP /np /log:$dest\group_policy\grouppolicy-robocopy-log1.txt'
	iex "& $command"
	}
foreach ($gpp in $gpp2){
	$source = $scheduledtask
	$command = 'robocopy.exe $source $destination /ZB /copy:DAT *.pol /r:0 /ts /FP /np /log:$dest\group_policy\grouppolicy-robocopy-log2.txt'
	iex "& $command"
	}
foreach ($gpp in $gpp3){
	$source = $scheduledtask
	$command = 'robocopy.exe $source $destination /ZB /copy:DAT *.ini /r:0 /ts /FP /np /log:$dest\group_policy\grouppolicy-robocopy-log3.txt'
	iex "& $command"
	}
...
$VERSION = (gwmi win32_OperatingSystem).Version
	$localprofiles = Get-WMIObject Win32_UserProfile -filter "Special != 'true'" -ComputerName $target | Where {$_.LocalPath -and ($_.ConvertToDateTime($_.LastUseTime)) -gt (get-date).AddDays(-15) }  #Can modify for number of days or all users
		...
		If ($Version -lt "5.4")
			...
			$command = 'robocopy.exe $userpath\$user\Recent $UserData\Recent /ZB /copy:DAT /r:0 /ts /FP /np /E /log:$dest\users\$user\robocopy-log_recent.txt'
			iex "& $command"
			}
		Else 
			{
			$userpath = "C:\Users"
			$command = 'robocopy.exe $userpath\$user\AppData\Roaming\Microsoft\Windows\Recent $UserData\Recent /ZB /copy:DAT /r:0 /ts /FP /np /E /log:$dest\users\$user\robocopy-log_recent.txt'
			iex "& $command"
			}
		...
		If ($Version -lt "5.4") 
			{
			$userpath = "C:\Documents and Settings"
			$command = 'robocopy.exe "$userpath\$user\Application Data\Microsoft\Office\Recent" $UserData\Recent /ZB /copy:DAT /r:0 /ts /FP /np /E /log:$dest\users\$user\robocopy-log_office-recent.txt'
			iex "& $command"
			}
		Else 
			{
			$userpath = "C:\Users"
			$command = 'robocopy.exe $userpath\$user\AppData\Roaming\Microsoft\Office\Recent $UserData\Office_Recent /ZB /copy:DAT /r:0 /ts /FP /np /E /log:$dest\users\$user\robocopy-log_office-recent.txt'
			iex "& $command"
			}
		...
		If ($Version -lt "5.4") 
			{
			$userpath = "C:\Documents and Settings"
			$command = 'robocopy.exe $userpath\$user\Nethood $UserData\Network_Recent /ZB /copy:DAT /r:0 /ts /FP /np /E /log:$dest\users\$user\robocopy-log_network-recent.txt'
			iex "& $command"
			}
		Else 
			{
			$userpath = "C:\Users"
			$command = 'robocopy.exe "$userpath\$user\AppData\Roaming\Microsoft\Windows\Network Shortcuts" $UserData\Network_Recent /ZB /copy:DAT /r:0 /ts /FP /np /E /log:$dest\users\$user\robocopy-log_network-recent.txt'
			iex "& $command"
			}
		...
		If ($Version -lt "5.4") 
			{
			$userpath = "C:\Documents and Settings"
			$command = 'robocopy.exe "$userpath\$user\Local Settings\Temp" $UserData\temp /ZB /copy:DAT /r:0 /ts /FP /np /E /log:$dest\users\$user\robocopy-log_temp.txt'
			iex "& $command"
			}
		Else 
			{
			$userpath = "C:\Users"
			$command = 'robocopy.exe $userpath\$user\AppData\Local\Temp $UserData\temp /ZB /copy:DAT /r:0 /ts /FP /np /E /log:$dest\users\$user\robocopy-log_temp.txt'
			iex "& $command"
			}
		...
		# Collecting Temporary Internet Files Folder 
			...
			{
			$userpath = "C:\Documents and Settings"
			$command = 'robocopy.exe "$userpath\$user\Local Settings\Temporary Internet Files" $UserData\Temporary_Internet_Files /ZB /copy:DAT /r:0 /ts /FP /np /E /log:$dest\users\$user\robocopy-log_tif.txt'
			iex "& $command"
		...
		Else 
			{
			$userpath = "C:\Users"
			$command = 'robocopy.exe "$userpath\$user\AppData\Local\Microsoft\Windows\Temporary Internet Files" $UserData\Temporary_Internet_Files /ZB /copy:DAT /r:0 /ts /FP /np /E /log:$dest\users\$user\robocopy-log_tif.txt'
			iex "& $command"		
			}
		...
		# Collecting the PrivacIE folder 
			...
			{
			$userpath = "C:\Documents and Settings"
			$command = "robocopy.exe $userpath\$user\PrivacIE $UserData\PrivacIE /ZB /copy:DAT /r:0 /ts /FP /np /E /log:$dest\users\$user\robocopy-log_privacie.txt"
			iex "& $command"	
			}
		Else 
			{
			$userpath = "C:\Users"
			$command = "robocopy.exe $userpath\$user\AppData\Roaming\Microsoft\Windows\PrivacIE $UserData\PrivacIE /ZB /copy:DAT /r:0 /ts /FP /np /E /log:$dest\users\$user\robocopy-log_privacie.txt"
			iex "& $command"	
			}
		...
		# Collecting the Cookies 
			...
			{
			$userpath = "C:\Documents and Settings"
			$command = "robocopy.exe $userpath\$user\Cookies $UserData\Cookies /ZB /copy:DAT /r:0 /ts /FP /np /E /log:$dest\users\$user\robocopy-log_cookies.txt"
			iex "& $command"
			}
		Else 
			{
			$userpath = "C:\Users"
			$command = "robocopy.exe $userpath\$user\AppData\Roaming\Microsoft\Windows\Cookies $UserData\Cookies /ZB /copy:DAT /r:0 /ts /FP /np /E /log:$dest\users\$user\robocopy-log_cookies.txt"
			iex "& $command"
			}
		...
		If ($Version -lt "5.4") 
			{
			$userpath = "C:\Documents and Settings"
			$command = "robocopy.exe $userpath\$user\ApplicationData\Sun\Java\Deployment\cache $UserData\Java_Cache /ZB /copy:DAT /r:0 /ts /FP /np /E /log:$dest\users\$user\robocopy-log_java.txt"
			iex "& $command"
			}
		Else 
			{
			$userpath = "C:\Users"
			$command = "robocopy.exe $userpath\$user\AppData\LocalLow\Sun\Java\Deployment\cache $UserData\Java_Cache /ZB /copy:DAT /r:0 /ts /FP /np /E /log:$dest\users\$user\robocopy-log_java.txt"
			iex "& $command"
			}
		}		
